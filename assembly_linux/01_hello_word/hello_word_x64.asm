segment .data         ;aqui vamos criar algo como de fosse um define, ou seja ao inves de precisar digitar 0xA = 10 e represena a quebra de linha, no restante do codigo usaremos lf 
	...
	sys_read equ 0x3  ;Operação de leitura              | --> rax
	sys_write equ 0x4 ;Operação de escrita              |
	ret_exit equ 0x0  ;Operação realizada com sucesso |
	std_out equ 0x1   ;Saida pardrão                  | --> rbx
...
_start:                 ;E o ponto onde inicia o codigo
    mov rax,sys_write   ;Indicamos em rax que e um dos registradores, o valor de 0x4 = 4 que está representado sys_write, quando eax está com o valor 4, o sistema vai escrever (ter uma saida)
    mov rbx,std_out     ;Semelhantemente o de cima, em rbx vamos atribuir o valor de 0x1 = 1, que siginifica que vamos usar a saida padrão 1 que e o nosso terminal
    mov rcx,msg1        ;Aqui atribuimos a rcx o que queremos mostrar na nossa saida, no caso nossa mensagem hello word
    mov rdx,len1        ;Atribuimos a rdx o inicio/tamanho da nossa str(mensagem)
    ...
    mov rax,sys_exit    ;Determinamos que o codigo vai encerrar/terminar
    mov rbx,ret_exit    ;Indica que não ouve erro na execução do codigo
    int sys_call